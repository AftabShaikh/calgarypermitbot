...
# Step 14: Finalize and save configuration
echo "📋 Finalizing configuration with all detected resources..."

# Ensure critical variables have values (use defaults if not set)
STORAGE_CONTAINER="${STORAGE_CONTAINER:-content}"
COSMOS_DATABASE="${COSMOS_DATABASE:-chathistory}"
COSMOS_CONTAINER="${COSMOS_CONTAINER:-chatcontainer}"
SEARCH_SKU="${SEARCH_SKU:-basic}"
OPENAI_SKU="${OPENAI_SKU:-S0}"

# Debug: Show final configuration before saving
...
echo "💾 Saving updated configuration with detected resources..."
cat > /tmp/deployment-config.env << EOF
# Updated configuration with detected resources
# Generated by 01-prepare-resources.sh on $(date)

# Basic Configuration (detected/verified)
export RESOURCE_GROUP="$RESOURCE_GROUP"
export LOCATION="$LOCATION"
export SUBSCRIPTION_ID="$SUBSCRIPTION_ID"

# App Service Configuration (detected/created)
export APP_SERVICE_PLAN="$APP_SERVICE_PLAN"
export APP_SERVICE_SKU="$APP_SERVICE_SKU"
export BACKEND_APP_NAME="$BACKEND_APP_NAME"
export FRONTEND_APP_NAME="$FRONTEND_APP_NAME"

# Storage Configuration (detected/created)
export STORAGE_ACCOUNT="$STORAGE_ACCOUNT"
export STORAGE_CONTAINER="content"

# Azure AI Search Configuration (detected/created)
export SEARCH_SERVICE="$SEARCH_SERVICE"
export SEARCH_SKU="${SEARCH_SKU:-basic}"

# Azure OpenAI Configuration (detected/created)
export OPENAI_SERVICE="$OPENAI_SERVICE"
export OPENAI_SKU="${OPENAI_SKU:-S0}"
export OPENAI_UNAVAILABLE="$OPENAI_UNAVAILABLE"

# Cosmos DB Configuration (detected/created)
export COSMOS_ACCOUNT="$COSMOS_ACCOUNT"
export COSMOS_DATABASE="chathistory"
export COSMOS_CONTAINER="chatcontainer"

# Connection Information (generated)
...
# Deployment Settings (inherited from config)
export VERBOSE_OUTPUT="${VERBOSE_OUTPUT:-false}"
export WAIT_TIMEOUT_MINUTES="${WAIT_TIMEOUT_MINUTES:-15}"
export DATA_FOLDER="${DATA_FOLDER:-data}"
export PYTHON_VERSION="${PYTHON_VERSION:-3.11}"
export NODE_VERSION="${NODE_VERSION:-18}"
export TAGS="${TAGS:-Environment=Production Project=CalgarypermitBot}"

echo "# Configuration file generated successfully on $(date)"
EOF
